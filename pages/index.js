...
export default function ZOE() {
  ...
  async function iniciar() {
    ...
    try {
      ...
      session.on('session.stream_ready', async () => {
        ...
        // CRUCIAL: ElevenLabs connector necesita el mic publicado
  ...
  return (
    <>
      <style>{`* { margin:0; padding:0; box-sizing:border-box; } body { background:#060610; font-family:Inter,sans-serif; color:#fff; } @keyframes pulse { 0%,100%{opacity:1;} 50%{opacity:0.4;} }`}</style>
      <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'2rem',position:'relative'}}>
        ...
        <div style={{position:'relative',zIndex:1,width:'100%',maxWidth:'960px',display:'flex',flexDirection:'column',alignItems:'center'}}>
          ...
          {estado === 'idle' && (
            <button onClick={iniciar} style={{background:'linear-gradient(135deg,#4f46e5,#7c3aed)',color:'#fff',border:'none',padding:'18px 56px',borderRadius:'100px',fontSize:'1rem',fontWeight:600,letterSpacing:'1px',cursor:'pointer',boxShadow:'0 0 50px rgba(79,70,229,0.5)'}}>Iniciar sesión en vivo</button>
          )}
          ...
          <div style={{display:estado==='live'?'flex':'none',width:'100%',flexDirection:'column',alignItems:'center',gap:'1.5rem'}}>
            <div style={{width:'100%',aspectRatio:'16/9',borderRadius:'20px',overflow:'hidden',boxShadow:'0 0 80px rgba(79,70,229,0.3)',position:'relative',background:'#0a0a1a'}}>
              ...
              <div style={{position:'absolute',top:'14px',left:'14px',background:'rgba(239,68,68,0.9)',color:'white',fontSize:'0.65rem',fontWeight:700,letterSpacing:'2px',padding:'5px 12px',borderRadius:'100px',display:'flex',alignItems:'center',gap:'6px'}}><div style={{width:'5px',height:'5px',background:'white',borderRadius:'50%',animation:'pulse 1.5s infinite'}}/>EN VIVO</div>
            </div>
            <button onClick={reconectar} style={{background:'rgba(255,255,255,0.08)',color:'rgba(255,255,255,0.5)',border:'1px solid rgba(255,255,255,0.15)',padding:'10px 32px',borderRadius:'100px',fontSize:'0.8rem',cursor:'pointer',letterSpacing:'2px'}}>RECONECTAR</button>
          </div>

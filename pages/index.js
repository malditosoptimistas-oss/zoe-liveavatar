          ...
          room.remoteParticipants.forEach((participant) => {
            participant.trackPublications.forEach((pub) => {
              if (pub.isSubscribed && pub.track) {
                if (pub.kind === 'video') { pub.track.attach(videoRef.current); setEstado('live'); }
                else if (pub.kind === 'audio') { const a = pub.track.attach(); document.body.appendChild(a); }
              }
            });
          });
          room.on('trackSubscribed', (track) => {
            if (track.kind === 'video') { track.attach(videoRef.current); setEstado('live'); }
            else if (track.kind === 'audio') { const a = track.attach(); document.body.appendChild(a); }
          });
          await room.localParticipant.setMicrophoneEnabled(true);
        } catch(e) { console.error(e); }
      });
      session.on('session.state_changed', (s) => { if (s === 'DISCONNECTED') setEstado('idle'); });
